...
def make_subfigures(n_L, material, n, magnitudes, figure_title, packed, share_ax = 'tot', Visible_labels = True): #Añadir que coja los titulos de eje según el nombre de archivo. EiP, lam
    fig_ = {}
    axs_ = {}
...
def pos_leg(leg, nrows, ncols, n_L):           #Está mal. Posición relativa! Solo funciona bien para n_L = 5
    if leg == 1 or leg == 2:
...
def set_legend(ax_, fig_, mode, lineas_unicas_, color_max_, max_R_values, nrows, ncols):
    if mode == 1:
    ...
    if mode == 2:
        ...
        leg_1= fig_.legend(handles = handles_1, loc='center', mode = 'expand', bbox_to_anchor= [pos_leg(1, nrows, ncols)[0], pos_leg(1, nrows, ncols)[1], 0.14,0], labelspacing=0.2,  fontsize = 'medium') #bbox_to_anchor=(0.5, 0.0), borderaxespad=0.1. Distance of a legend from the bounding box edge is set by the borderaxespad argument
        ...
        etiquetas = ['Re', 'Im']
        leg_2 = fig_.legend(lineas_unicas_, etiquetas, loc='center', bbox_to_anchor= [pos_leg(2, nrows, ncols)[0], pos_leg(2, nrows, ncols)[1]], labelspacing=0.2,  fontsize = 'medium', frameon = False) 
